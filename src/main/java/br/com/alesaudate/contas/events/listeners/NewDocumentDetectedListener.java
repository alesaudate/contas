...
public class NewDocumentDetectedListener extends GenericMessageListener<DocumentFileEvent> {
    ...
    public void accept(DocumentFileEvent event) throws IOException {
        ...
        if (data.getEntries().isEmpty()) {
            io.tell("Todas as entradas do documento já foram tratadas. Finalizando o procedimento...");
            saveDocumentListener.accept(event);
        } else {
            boolean review = io.askBoolean("Você quer revisar as entradas?", io.asSet("Sim", "Quero"), event.getFile());
            if (review) {
